            ...
            <div class="container">
                <el-card :key="utisak.id_utiska" class="utisak_card" v-for="utisak in utisci">
                    <div class="UtisakHeader">
                        <h1>{{utisak.proizvod}}</h1>
                        <el-button v-if="korisnikAutor == utisak.id_utiska"
                            @click="deleteUtisak(utisak.id_utiska)" type="danger"
                            icon="el-icon-delete" circle size="mini" 
                            style="border-color:rgba(213, 34, 92, 0.925); background-color:rgba(213, 34, 92, 0.925);"></el-button>
                    </div>
                    <div>
                        <p>{{utisak.tekst}}</p>
                    </div>
...
<style scoped>
    ...
    .utisak-card {
        ...
        margin-top: 1.5em;
        overflow: auto;
        background-color: rgba(249, 227, 254, 0.938);
        opacity: 0.8;
